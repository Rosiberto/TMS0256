...
class LoginModel extends Database {
    ...
    public function fetchClienteByLoginId($loginId) {
        $stm = $this->pdo->prepare("SELECT * FROM cliente WHERE fk_Login_ID = ?");
        $stm->execute([$loginId]);
        return $stm->fetch(PDO::FETCH_ASSOC);
    }

    public function fetchEmpregadoByLoginId($loginId) {
        $stm = $this->pdo->prepare("SELECT * FROM empregado WHERE fk_Login_ID = ?");
        $stm->execute([$loginId]);
        return $stm->fetch(PDO::FETCH_ASSOC);
    }
